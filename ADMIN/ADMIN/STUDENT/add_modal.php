				...
				<div class="row form-group">
					...
					<div class="col-sm-10">
						<select class="form-control" name="code">
							...
							$sqls = "SELECT *,grade.year as grade, grade.id as grade_year FROM grade LEFT JOIN section ON grade.id = section.year";

							//use for MySQLi-OOP
							$query = $conn->query($sqls);
							while($row = $query->fetch_assoc()){
						     ?>		
							 <option value="<?php echo $row['code']; ?>">GRADE : <?php echo $row['grade']; ?> | SECTION : <?php echo strtoupper($row['section']); ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
