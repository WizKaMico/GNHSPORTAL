			...
			<form method="POST" action="edit.php">
				...
				<div class="row form-group">
					...
					<div class="col-sm-10">
							...
							$sqls = "SELECT *,grade.year as grade, grade.id as grade_year, section.id as sec_id FROM grade LEFT JOIN section ON grade.id = section.year";

							//use for MySQLi-OOP
							$queries = $conn->query($sqls);
							while($xrow = $queries->fetch_assoc()){
						     ?>		
							 <option value="<?php echo $xrow['code']; ?>">GRADE : <?php echo $xrow['grade']; ?> | SECTION : <?php echo strtoupper($xrow['section']); ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">	
            	<center>
            		<div id="content">
            		<img src="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=http%3A%2F%2Flocalhost/GNHS/PAGES/STUDENT/index.php?email=<?php echo $row['email']; ?>%2F&choe=UTF-8.png" title="STUDENT:<?php echo $row['name']; ?>|EMPLOYEENO:<?php echo $row['employee_no']; ?>"/>
            	   </div>
            	   	</center>
            	  <div id="editor"></div>
            ...
            <div class="modal-footer">
            	<button id="cmd" class="btn btn-default">DOWNLOAD</button>
            	<!-- <a href="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=http%3A%2F%2Flocalhost/GNHS/PAGES/STUDENT/index.php?email=<?php echo $row['email']; ?>%2F&choe=UTF-8" class="btn btn-default" download>
            	DOWNLOAD</a> -->
                <button type="button" class="btn btn-default" data-dismiss="modal"><span class="glyphicon glyphicon-remove"></span> Cancel</button>
            </div>
				...
				<div class="row form-group">
					...
					<div class="col-sm-10">
						<select class="form-control" name="subject_id">
							<?php
							$sql1 = "SELECT *,module_details.code as MCODE FROM module_details LEFT JOIN subject ON module_details.subject_id = subject.id";
							$querys = $conn->query($sql1);
							while($rows = $querys->fetch_assoc()){
							?>	
							<option value="<?php echo $rows['MCODE']; ?>"> <?php echo $rows['module_name']; ?> | <?php echo $rows['subject_name']; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
