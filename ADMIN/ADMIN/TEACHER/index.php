				...
				<table id="myTable" class="table table-bordered table-striped">
					...
					<tbody>
						<?php
							...
							$sql = "SELECT *, grade.year as GRADEHANDLE, section.section as SECTIONHANDLE FROM teacher LEFT JOIN section ON teacher.department = section.code LEFT JOIN grade ON section.year = grade.id WHERE teacher.status != 'ARCHIVE'";

							// SELECT *, grade.year as GRADEHANDLE, section.section as SECTIONHANDLE FROM teacher LEFT JOIN section ON teacher.department = section.code LEFT JOIN grade ON section.year = grade.id WHERE teacher.status != 'ARCHIVE'
							...
							while($row = $query->fetch_assoc()){
								...
								"<tr>
									...
									<td>".$row['fname']."</td>
									<td>".$row['initial']."</td>
									<td>".$row['GRADEHANDLE']." | ".$row['SECTIONHANDLE']."</td>
									<td>".$row['email']."</td>
									<td>
										...
										<a href='assigned.php?department=".$row['department']."&teacher=".$row['fname']." ".$row['lname']."' class='btn btn-danger btn-sm' style='background-color:#421714; border-color:#421714;'><span class='glyphicon glyphicon-user'></span></a>
									</td>
								</tr>";
